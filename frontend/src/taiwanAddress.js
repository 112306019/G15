...
export const TAIWAN_ADDRESS_DATA = Object.fromEntries(
  Object.entries(RAW_ADDRESS_DATA).map(([city, items]) => [
    city,
    items.map(([district, postalCode]) => ({
      district,
      postalCode,
    })),
  ])
...
export const TAIWAN_CITIES =
  Object.keys(TAIWAN_ADDRESS_DATA)
...
export function getPostalCode(
  ...
  const matched =
    getDistrictsByCity(city).find(
      item =>
        item.district === district
    )

  return matched?.postalCode || ''
}