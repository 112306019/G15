...
export default function CheckoutPage({
  ...
  // 信用卡驗證
  // ============================
  const cardNumDigits = useMemo(
    () => digitsOnly(cardNum),
    [cardNum]
  );

  const expiryDigits = useMemo(
    () => digitsOnly(expiry),
    [expiry]
  );
  ...
  const inputNormal =
    "bg-white border-[#E2DDD4] focus:border-[#1A1A18]";

  const inputValid =
    "bg-white border-[#6BBF6B]";

  const inputClass = (valid) =>
    `${inputBase} ${
      valid ? inputValid : inputNormal
    }`;
    ...
    <div className="min-h-screen bg-[#F5F0E8] text-[#1A1A18] font-serif">
      ...
      <div className="mx-auto max-w-[1000px] px-6 pb-20 pt-12 grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-10 lg:gap-14 items-start">
        ...
        <div>
          ...
          <p className="font-['DM_Serif_Display'] text-[16px] font-bold mb-6">
            Credit Card
          </p>
