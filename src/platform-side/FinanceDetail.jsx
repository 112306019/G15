...
export default function FinanceDetail({ setCurrentPage, finance }) {
  ...
  // 模擬點擊撥款的動作
  const handlePayout = () => {
    setShowConfirmModal(false); // 🌟 按下確認後，先把彈窗關掉
    setIsProcessing(true);      // 🌟 按鈕變成「處理中」
    
    setTimeout(() => {
      alert(`✅ 已成功匯款 $${finance.amount.toLocaleString()} 至 ${finance.kocName} 的帳戶！`);
      setIsProcessing(false);
      setCurrentPage('finance'); // 撥款完自動回到列表
    }, 1500);
  };
    ...
    <div className="relative p-8 max-w-5xl mx-auto space-y-6">
          ...
          <div className="bg-white rounded-2xl p-8 max-w-sm w-full shadow-2xl transform transition-all">
            ...
            <div className="flex gap-3">
              ...
              <button 
                ...
                className="flex-1 px-4 py-2.5 bg-black text-white rounded-lg font-bold hover:bg-gray-800 transition-colors"
              >
                確認撥款
              </button>
            </div>
      ...
      <div className="flex items-center justify-between mb-2">
        ...
        <span className={`px-4 py-1.5 rounded-full text-sm font-bold ${
          finance.status === 'completed' ? 'bg-green-100 text-green-600' : 'bg-orange-100 text-orange-600'
        }`}>
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <div className="col-span-2 bg-white rounded-2xl shadow-sm border border-gray-200 p-8 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <p className="text-sm text-gray-500 mb-1">申請 KOC</p>
              <p className="font-bold text-gray-800">{finance.kocName}</p>
            </div>
            <div>
              <p className="text-sm text-gray-500 mb-1">申請日期</p>
              <p className="font-bold text-gray-800">{finance.date}</p>
            </div>
            <div className="col-span-2">
              <p className="text-sm text-gray-500 mb-1">任務名稱</p>
              <p className="font-bold text-gray-800">{finance.taskName}</p>
            </div>
        ...
        <div className="col-span-1 bg-white rounded-2xl shadow-sm border border-gray-200 p-8 flex flex-col justify-between">
          <div className="space-y-4">
            ...
            <div className="bg-orange-50 p-4 rounded-xl border border-orange-100 space-y-3">
              <div>
                <p className="text-xs text-orange-600/70 font-bold mb-1">銀行代碼 / 名稱</p>
                <p className="font-bold text-gray-800">{finance.bank}</p>
              </div>
              <div>
                <p className="text-xs text-orange-600/70 font-bold mb-1">銀行帳號</p>
                <p className="font-mono font-bold text-lg text-gray-800">{finance.account}</p>
              </div>
              <div>
                <p className="text-xs text-orange-600/70 font-bold mb-1">戶名</p>
                <p className="font-bold text-gray-800">{finance.kocName}</p>
              </div>
            </div>
          </div>
