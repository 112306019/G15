  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex items-center justify-between bg-white p-4 rounded-t-xl border border-gray-200 border-b-0">
        <input 
          type="text" placeholder="搜尋任務名稱、發布廠商..." 
            ...
            {isLoading ? (
              ...
              filteredTasks.map(t => (
                <tr key={t.id} className="hover:bg-indigo-50/50 transition-colors">
                  ...
                  <td className="py-4 px-6 align-middle text-center font-bold text-lg text-indigo-600">${t.budget.toLocaleString()}</td>
                  ...
                  <td className="py-4 px-6 align-middle text-center">
                    {/* 🌟 替換成 StatusBadge */}
                    <StatusBadge type={t.status === 'recruiting' ? 'info' : t.status === 'ongoing' ? 'warning' : t.status === 'completed' ? 'success' : 'danger'}>
                      {t.status === 'recruiting' ? '📣 招募中' : t.status === 'ongoing' ? '🏃 進行中' : t.status === 'completed' ? '✅ 已結案' : '🚫 已取消'}
                    </StatusBadge>
                  </td>
                  <td className="py-4 px-6 align-middle text-center">
                    <button 
                      onClick={() => { setSelectedTask(t); setCurrentPage('taskDetail'); }} 
