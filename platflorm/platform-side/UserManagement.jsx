...
export default function UserManagement({ setCurrentPage }) {
  ...
  useEffect(() => {
    setTimeout(() => {
      setUsers([
        { id: 'U001', name: '陳曉明', phone: '0912-345-678', date: '2026/01/05', email: 'xiaoming@gmail.com', status: 'applying' },
        { id: 'U002', name: '林心如', phone: '0922-333-444', date: '2026/01/12', email: 'ruby_lin@yahoo.com.tw', status: 'active' },
        { id: 'U003', name: '張建國', phone: '0933-111-222', date: '2025/11/20', email: 'jk_zhang@hotmail.com', status: 'suspended' }
      ]);
      setIsLoading(false);
    }, 500);
  }, []);
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex items-center justify-between bg-white p-4 rounded-t-xl border border-gray-200 border-b-0">
        <input type="text" placeholder="搜尋姓名、Email..." className="pl-4 pr-4 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm w-80"
          value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        <select className="border border-gray-300 px-4 py-2 rounded-lg text-sm bg-white" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}>
            ...
            {isLoading ? <tr><td colSpan="5" className="py-8 text-center text-gray-500">載入中...</td></tr> :
              filteredUsers.map(u => (
                <tr key={u.id} className="hover:bg-gray-50">
                  <td className="py-4 px-6 align-middle font-bold">{u.name}</td>
                  <td className="py-4 px-6 align-middle"><p>{u.phone}</p><p className="text-xs text-gray-400">{u.email}</p></td>
                  ...
                  <td className="py-4 px-6 align-middle text-center">
                    <span className={`px-3 py-1.5 rounded-full font-bold text-xs whitespace-nowrap inline-block ${
                      u.status === 'active' ? 'bg-green-100 text-green-600' : u.status === 'applying' ? 'bg-orange-100 text-orange-600' : 'bg-red-100 text-red-600'
                    }`}>{u.status === 'active' ? '已啟用' : u.status === 'applying' ? '申請中' : '已停權'}</span>
                  </td>
                  <td className="py-4 px-6 align-middle text-center">
